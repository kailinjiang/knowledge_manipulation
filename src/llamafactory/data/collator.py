...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    use_kore_loss: bool = False

    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # `supervised.py` may always create `rats_*` keys (filled with None when absent).
        # Transformers' padding will try to tensorize them and crash on None, so we must
        # only collate `rats_*` when Kore loss is enabled.
        if not self.use_kore_loss:
            for feature in features:
                feature.pop("rats_input_ids", None)
                feature.pop("rats_attention_mask", None)
            features = super().__call__(features)
        else:
            # If Kore loss is enabled, features may contain rats_input_ids/rats_attention_mask.
            # We collate the base features first, then collate the rats features and attach them with `rats_` prefix.
            has_rats = any(feature.get("rats_input_ids") is not None for feature in features)
            if has_rats:
                base_features: list[dict[str, Any]] = []
                rats_features: list[dict[str, Any]] = []
                for feature in features:
                    base_feature = feature.copy()
                    rats_input_ids = base_feature.pop("rats_input_ids", None)
                    rats_attention_mask = base_feature.pop("rats_attention_mask", None)
                    base_features.append(base_feature)

                    if rats_input_ids is None:
                        # Fallback to base input_ids to keep tensor shapes consistent
                        rats_input_ids = base_feature["input_ids"]
                        rats_attention_mask = base_feature.get("attention_mask")

                    rats_feature = base_feature.copy()
                    rats_feature["input_ids"] = rats_input_ids
                    if rats_attention_mask is not None:
                        rats_feature["attention_mask"] = rats_attention_mask
                    rats_features.append(rats_feature)

                base_batch = super().__call__(base_features)
                rats_batch = MultiModalDataCollatorForSeq2Seq.__call__(self, rats_features)

                # Attach text-dependent tensors for rats forward.
                base_batch["rats_input_ids"] = rats_batch["input_ids"]
                base_batch["rats_attention_mask"] = rats_batch["attention_mask"]
                if "cross_attention_mask" in rats_batch:
                    base_batch["rats_cross_attention_mask"] = rats_batch["cross_attention_mask"]
                if "token_type_ids" in rats_batch:
                    base_batch["rats_token_type_ids"] = rats_batch["token_type_ids"]
                if "position_ids" in rats_batch:
                    base_batch["rats_position_ids"] = rats_batch["position_ids"]
                if "rope_deltas" in rats_batch:
                    base_batch["rats_rope_deltas"] = rats_batch["rope_deltas"]

                features = base_batch
            else:
                # Kore loss enabled but this batch has no rats: drop the placeholder keys to avoid None padding.
                for feature in features:
                    feature.pop("rats_input_ids", None)
                    feature.pop("rats_attention_mask", None)
                features = super().__call__(features)
