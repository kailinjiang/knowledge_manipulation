...
class FinetuningArguments(
    ...
    use_kore_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the Kore loss (CE + KL(p||q) with/without rationales)."},
    )
    kore_alpha: float = field(
        default=1.0,
        metadata={"help": "The alpha parameter for Kore loss to control the weight of KL(p||q)."},
    )
    kore_beta: float = field(
        default=0.0,
        metadata={
            "help": (
                "The beta parameter for Kore loss to control the weight of KL(base||p). "
                "This term aligns the current model distribution to the base (reference) model on no-rats inputs."
            )
        },
    )
