...
def run_sft(
    ...
    ref_model = None
    if finetuning_args.use_asft_loss or (finetuning_args.use_kore_loss and finetuning_args.kore_beta > 0):
        # For Kore's base alignment term, we need a frozen reference model even in LoRA mode.
        # `create_ref_model` returns None for LoRA by default, so we build a temporary finetuning args
        # that forces creating a base model when no explicit `ref_model` is provided.
        if finetuning_args.ref_model is None and finetuning_args.finetuning_type == "lora":
            ref_finetuning_args = FinetuningArguments()
            ref_finetuning_args.finetuning_type = "full"
            ref_model = create_ref_model(model_args, ref_finetuning_args)
        else:
    ...
    data_collator = SFTDataCollatorWith4DAttentionMask(
        ...
        use_kore_loss=finetuning_args.use_kore_loss,
        **tokenizer_module,
    )
